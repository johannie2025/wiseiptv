...
public class PlayerEngine {
    ...
    public PlayerEngine(Context ctx, Listener l) {
        this.context = ctx.getApplicationContext(); this.listener = l; buildPlayer();
    }
    private void buildPlayer() {
        ...
        exoPlayer = new ExoPlayer.Builder(context)
            .setLoadControl(lc).setTrackSelector(ts)
            .setMediaSourceFactory(new DefaultMediaSourceFactory(dsFactory)).build();
        exoPlayer.addListener(new Player.Listener() {
            @Override public void onPlaybackStateChanged(int s) {
                if (s == Player.STATE_BUFFERING) listener.onBuffering(true);
                else if (s == Player.STATE_READY) { listener.onBuffering(false); listener.onPlaying(); }
            }
            @Override public void onPlayerError(PlaybackException e) {
                Log.e(TAG, e.getMessage());
                listener.onError(e.getMessage() != null ? e.getMessage() : "Erreur de lecture");
            }
        });
    }
    public void attachView(PlayerView v) { v.setPlayer(exoPlayer); }
    public void play(String url) {
        if (url == null || url.isEmpty()) return;
        exoPlayer.setMediaItem(MediaItem.fromUri(Uri.parse(url)));
        exoPlayer.setPlayWhenReady(true); exoPlayer.prepare(); applyQuality(currentQuality);
    }
